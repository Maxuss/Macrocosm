...
class HighSkillEmitter(role: Role, channel: Channel) :
    ...
    override fun handle(subject: Context, bot: JDA) {
        dedicatedChannel.sendMessageEmbeds(Discord.embed {
            ...
            addField(
                "**${subject.skill.emoji} ${subject.skill.inst.name} Level _${subject.level}_ Reached**",
                "${
                    Discord.getAuthenticatedOrNull(subject.player)?.asMention ?: "${
                        subject.player.rank.format.str().stripTags()
                    } ${subject.player.paper?.name}"
                } is now `${subject.skill.profession} LVL ${subject.level}`!",
                false
            )
            addField(
                "**Total Experience**",
                "**${
                    Formatting.withCommas(
                        subject.skill.inst.table.totalExpForLevel(subject.level).toBigDecimal()
                    )
                } EXP**${if (subject.skill.maxLevel == subject.level) "MAX LEVEL!" else ""}",
                false
            )
