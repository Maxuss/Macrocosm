...
fun shapedRecipe(id: String, result: MacrocosmItem, matrix: List<String>, vararg ingredients: Pair<Char, Pair<Identifier, Int>>): SbRecipe = ShapedRecipe(id(id), matrix, ingredients.toMap(), result)

class ShapedRecipe(override val id: Identifier, pattern: List<String>, private val ingredientMap: Map<Char, Pair<Identifier, Int>>, private val result: MacrocosmItem): SbRecipe {
    ...
    init {
        ...
        while(newPattern.size < 2)
            newPattern.add("   ")
        ingredients = newPattern.map { it.toCharArray().map { c -> if(c == ' ') Pair(Identifier.NULL, 0) else ingredientMap[c] ?: throw IllegalStateException("Character '$c' was not defined in the ingredient map!") } }
        // example
    ...
    override fun matches(player: MacrocosmPlayer, inv: Inventory, modify: Boolean): Pair<Boolean, HashMap<Int, Pair<ItemStack, Int>>> {
        ...
        for(x in 0..2) {
            for(y in 0..2) {
                ...
                val item = inv.getItem(itemIndex) ?: if(expectedId == Identifier.NULL) continue else return Pair(false, hashMapOf())
                if(item.type.isAir && expectedId == Identifier.NULL) continue
                val mcItem = item.macrocosm!! // asserting that the item is not null, because we checked that it is not air before
