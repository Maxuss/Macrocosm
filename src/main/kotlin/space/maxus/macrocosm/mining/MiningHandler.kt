...
private fun skillExpFromBlock(block: Block): Pair<Int, SkillType>? {
    ...
    val exp = meta ?: when(block.type) {
        ...
        Material.IRON_ORE, Material.NETHER_GOLD_ORE, Material.REDSTONE_ORE, Material.DEEPSLATE_COPPER_ORE, Material.DEEPSLATE_COAL_ORE -> return Pair(25, SkillType.MINING)
        Material.GOLD_ORE, Material.DEEPSLATE_IRON_ORE, Material.DEEPSLATE_REDSTONE_ORE -> return Pair(35, SkillType.MINING)
    ...
    return Pair(exp, SkillType.valueOf(block.getMetadata("SKILL_TYPE").firstOrNull()?.asString() ?: when(block.type) {
        // mining
        ...
        Material.IRON_ORE, Material.NETHER_GOLD_ORE, Material.REDSTONE_ORE, Material.DEEPSLATE_COPPER_ORE, Material.DEEPSLATE_COAL_ORE -> return Pair(exp, SkillType.MINING)
        Material.GOLD_ORE, Material.DEEPSLATE_IRON_ORE, Material.DEEPSLATE_REDSTONE_ORE -> return Pair(exp, SkillType.MINING)
        Material.DIAMOND_ORE, Material.EMERALD_ORE, Material.DEEPSLATE_GOLD_ORE -> return Pair(exp, SkillType.MINING)
        ...
        Material.SUGAR_CANE, Material.BROWN_MUSHROOM, Material.RED_MUSHROOM -> return Pair(exp, SkillType.FARMING)
...
private fun suitingTypes(block: Block): List<ItemType> {
    ...
    if(ty.name.contains("STONE") || ty.name.contains("DEBRIS") || ty.name.contains("DEEP") || ty.name.contains("ORE") || stones.any { ty.name.contains(it) })
...
object MiningHandler: PacketAdapter(Macrocosm, ListenerPriority.NORMAL, PacketType.Play.Client.ARM_ANIMATION), Listener {
    ...
    fun onMineTick(e: MineTickEvent) {
        ...
        if(state >= 1f) {
            ...
            val packet = ClientboundBlockDestructionPacket(bId, BlockPos(breaking.x, breaking.y, breaking.z), floor(9 * state).roundToInt())
            e.player.sendPacket(packet)
        }
    }
