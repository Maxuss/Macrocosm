...
object EnderDragonPet: Pet(
    ...
    listOf(
        ...
        PetAbility("Void Conqueror", "Buffs <gold>Dragon Sets<gray> and <gold>Void Scepter<gray> by <red>+[1] ${Statistic.STRENGTH.display}<gray>."),
        PetAbility("Extinction", "You deal <red>[2]%<gray> more ${Statistic.DAMAGE.display}<gray>, when there are less than <green>3 enemies<gray> around.")
    ),
    stats {
        critDamage = 15f
        strength = 15f
    ...
    override val effects: PetEffects = FixedPetEffects(listOf(
    ...
    fun extinctionAbility(e: PlayerDealDamageEvent) {
        ...
        if(e.player.paper!!.getNearbyEntities(8.0, 8.0, 8.0).filter { it is LivingEntity && it !is ArmorStand }.size <= 3) {
