    ...
    override fun registerListeners() {
        listen<PlayerCalculateStatsEvent> { e ->
            ...
            val mod = 1f + PolarityAlignment.getNearbyAlignments(p, 8.0).map { (_, b) ->
                ...
                if (leftHand == PolarityAlignment.CHAOTIC && rightHand == PolarityAlignment.EVIL) {
                    .08f
                } else (if (leftHand == PolarityAlignment.CHAOTIC) 0.03f else 0f) + (if (rightHand == PolarityAlignment.EVIL) 0.03f else 0f)
            }.sum()
    ...
    override fun registerListeners() {
        listen<PlayerCalculateStatsEvent> { e ->
            ...
            val mod = 1f + PolarityAlignment.getNearbyAlignments(p, 8.0).map { (_, b) ->
                val (leftHand, rightHand) = b
                // find lawful or good
                if (leftHand == PolarityAlignment.LAWFUL && rightHand == PolarityAlignment.GOOD) {
                    .08f
                } else (if (leftHand == PolarityAlignment.LAWFUL) 0.03f else 0f) + (if (rightHand == PolarityAlignment.GOOD) 0.03f else 0f)
            }.sum()
            e.stats.multiply(mod)
        }
    }
...
object ForbiddenPolaritiesAbility : FullSetBonus(
    ...
    override fun registerListeners() {
        listen<PlayerCalculateStatsEvent> { e ->
            ...
            val p = e.player.paper ?: return@listen
            val alignments =
                PolarityAlignment.getNearbyAlignments(p, 8.0).filter { (_, b) -> b.first != PolarityAlignment.NEUTRAL }
    ...
    override fun registerListeners() {
        listen<PlayerCalculateStatsEvent> { e ->
            ...
            if (alignment == null || alignment.first == PolarityAlignment.CONVERGENCE)
                return@listen
            val confluxNearby = PolarityAlignment.getNearbyAlignments(p, 10.0)
                .filter { (_, b) -> b.first == PolarityAlignment.CONVERGENCE }.isNotEmpty()
            if (confluxNearby)
    ...
    override fun registerListeners() {
        listen<PlayerCalculateStatsEvent> { e ->
            if (!ensureSetRequirement(e.player))
                return@listen
            val p = e.player.paper ?: return@listen
            val alignments = PolarityAlignment.getNearbyAlignments(p, 8.0)
                .filter { (_, b) -> b.first != PolarityAlignment.CONVERGENCE }
...
private enum class PolarityAlignment(vararg sets: String) {
    ...
    companion object {
        fun alignmentOfPlayer(
            player: Player,
            set: List<Identifier> = player.getArmorIds()
        ): Pair<PolarityAlignment, PolarityAlignment>? {
            if (set.size != 4)
                return null
            return PolarityAlignment.values().filter { alignment ->
                alignment.sets.firstOrNull { alignSet -> set.all { piece -> piece.path.contains(alignSet) } }
                    ?.let { true } ?: false
            }
                .let { (if (it.isEmpty()) return null else if (it.size == 1) /* We got neutral or conflux */ it[0] to it[0] else it[0] to it[1]) }
        }

        fun getNearbyAlignments(
            around: Player,
            radius: Double
        ): HashMap<Player, Pair<PolarityAlignment, PolarityAlignment>> {
            val armorIds = around.getNearbyEntities(radius, radius, radius).filterIsInstance<Player>()
                .mapNotNull { val ids = it.getArmorIds(); if (ids.size != 4) null else it to ids }
...
internal fun exemptsEssence(name: String) =
    "<br><yellow>This set exempts the <italic>$name</italic><br><yellow>essence."
