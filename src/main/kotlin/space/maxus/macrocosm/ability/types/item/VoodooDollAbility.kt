...
object VoodooDollAbility: AbilityBase(AbilityType.RIGHT_CLICK, "Acupuncture", "Shoots arrows from every direction around the targeted enemy.<br>Hit entity is slowed down and takes <red>[3000:0.2] ${Statistic.DAMAGE.display}<gray>/s for <green>5s<gray>.") {
    ...
    override fun registerListeners() {
        listen<PlayerRightClickEvent> { e ->
            ...
            // spawning arrows
            for(i in 0..10) {
                val pos = Location(loc.world, loc.x + Random.nextSignedDouble() * 3, loc.y + Random.nextSignedDouble() * 3, loc.z + Random.nextSignedDouble() * 3)
