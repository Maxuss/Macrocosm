...
object OrderSetAbility: FullSetBonus("Blessing of Order",
    "Constructs an <green>aura<gray> around you, that boosts ${Statistic.VITALITY.display}<gray> by <green>75%<gray> for all players within <yellow>8 blocks<gray>.<br>For every player within <yellow>10 blocks<gray> you gain <red>+1.5% ${Statistic.HEALTH.display}<gray>.${exemptsEssence("<green>Devotedly Good")}"
) {
...
object EarthPolaritySetBonus: FullSetBonus("Blessing of Earth", "You take <green>30%<gray> less ${Statistic.DAMAGE.display}<gray> while standing <yellow>on ground<gray> but you deal <red>70%<gray> less ranged ${Statistic.DAMAGE.display}<gray>.${exemptsEssence("<green>Lawful <yellow>Neutral")}") {
    override fun registerListeners() {
...
object FirePolaritySetBonus: FullSetBonus("Blessing of Fire", "You deal <red>+50% Melee ${Statistic.DAMAGE.display}<gray> but your ${Statistic.INTELLIGENCE.display}<gray> is capped at <red>25%<gray> of your total ${Statistic.STRENGTH.display}<gray>.${exemptsEssence("<green>Lawful <red>Evil")}") {
    override fun registerListeners() {
        listen<PlayerDealDamageEvent> { e ->
