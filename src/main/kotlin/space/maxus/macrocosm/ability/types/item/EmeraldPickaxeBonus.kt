...
object EmeraldPickaxeBonus : AbilityBase(
    ...
    override fun buildLore(lore: MutableList<Component>, player: MacrocosmPlayer?) {
        ...
        tmp.add("".toComponent())
        if (player == null) {
            tmp.add(text("<gray>Emerald Collection: <green>0").noitalic())
            tmp.add(text("${Statistic.MINING_FORTUNE.display}<gray> Bonus: <green>0").noitalic())
        } else {
            val coll = player.collections[CollectionType.EMERALD]
            tmp.add(text("<gray>Emerald Collection: <green>$coll").noitalic())
