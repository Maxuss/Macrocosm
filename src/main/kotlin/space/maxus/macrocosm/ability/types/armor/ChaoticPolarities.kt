...
object SpiritsPolaritySetBonus: FullSetBonus("Blessing of Spirits", "Attacking enemies with <red>Melee<gray> restores <aqua>50 ${Statistic.INTELLIGENCE.specialChar} Mana<gray> and grants <red>+2.5% ${Statistic.ABILITY_DAMAGE.display}<gray> for <green>5 seconds <dark_gray>(capped at +50%)<gray>.<br> Your ${Statistic.VIGOR.display}<gray> is set to <red>0<gray>.${exemptsEssence("<red>Chaotic <green>Good")}") {
...
object VoidPolaritySetBonus: FullSetBonus("Blessing of Void", "Damaging <red>10 different enemies<gray> within <green>10 seconds<gray> summons a <dark_purple>Void Storm<gray>, dealing <red>500%<gray> of your weapon's normal ${Statistic.DAMAGE.display}<gray> to all <red>damaged<gray> enemies. You can not deal <yellow>any<gray> <red>Melee <gray>or <red>Ranged ${Statistic.DAMAGE.display}<gray> for the next <green>5 seconds<gray>.${exemptsEssence("<red>Chaotic <yellow>Neutral")}") {
    ...
    private fun doCounterTick(e: PlayerDealDamageEvent) {
        damaged.setOrTakeMut(e.player.ref) {
            ...
            if(damaged.size >= 10) {
                ...
                val damage = DamageCalculator.calculateStandardDealt(p.inventory.itemInMainHand.macrocosm!!.stats(e.player).damage, e.player.stats()!!)
                taskRunLater(1L) {
                ...
                damaged.forEach { entity ->
                    ...
                    mc.damage(DamageCalculator.calculateStandardReceived(damage.first, mc.calculateStats()), p, DamageKind.MAGIC)
...
object ChaosPolaritySetBonus2: FullSetBonus("Chaos Shield", "Kill <red>3 enemies<gray> to accumulate a layer of a <gold>Chaos Shield<dark_gray> (3 layers max)<gray>. The <gold>Chaos Shield<gray> returns <red>250%<gray> of ${Statistic.DAMAGE.display}<gray> received to the damager.${exemptsEssence("<red>Devotedly Chaotic")}") {
