...
class ZombieTalisman(applicable: String, private val reduction: Float): AccessoryAbility(applicable, "Reduces damage taken from Zombies by <green>${(reduction * 100f).roundToInt()}%<gray>.") {
...
class WitherTalisman(applicable: String, private val reduction: Float): AccessoryAbility(applicable, "Reduces damage taken from Withers by <green>${(reduction * 100f).roundToInt()}%<gray>.") {
...
class SpiderTalisman(applicable: String, private val reduction: Float): AccessoryAbility(applicable, "Reduces damage taken from Spiders by <green>${(reduction * 100f).roundToInt()}%<gray>.") {
...
class EnderTalisman(applicable: String, private val reduction: Float): AccessoryAbility(applicable, "Reduces damage taken from Ender Creatures by <green>${(reduction * 100f).roundToInt()}%<gray>.") {
