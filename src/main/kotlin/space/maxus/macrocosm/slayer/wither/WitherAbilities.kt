...
object WitherAbilities {
    ...
    fun summonBarrier(entity: LivingEntity, tier: Int) {
        ...
        val hologram: ArmorStand = entity.location.world.spawnEntity(entity.location, EntityType.ARMOR_STAND) as ArmorStand
        ...
        task(delay = 200L) {
            ...
            val summoner = Macrocosm.loadedPlayers.values.firstOrNull { player -> player.boundSlayerBoss == entity.uniqueId } ?: return@task
            particle(Particle.REDSTONE) {
        ...
        task(period = 200L) {
            applyToBosses { _, living, tier ->
                ...
                center.getNearbyPlayers(4.0).forEach { player ->
                    ...
                    val dmg = DamageCalculator.calculateStandardReceived(if(tier == 3) 2500f else 5000f, mc.stats() ?: return@forEach)
    ...
    val UNDERWORLD_UNSEALED = SlayerAbility(
        ...
        listen<PlayerDealDamageEvent> { e ->
            if(SlayerAbility.bosses[SlayerType.CINDERFLAME_SPIRIT]?.contains(e.damaged.uniqueId) != true || underworldUnsealed.contains(e.damaged.uniqueId))
                return@listen
            val mc = e.damaged.macrocosm ?: return@listen
            val id = mc.getId(e.damaged).path
            val tier = Integer.valueOf(id.replace("${slayerType.name.lowercase()}_", ""))
