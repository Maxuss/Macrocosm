...
fun rngMeterButton(slayer: SlayerLevel, slayerType: SlayerType): ItemStack {
    return itemStack(Material.PAINTING) {
        meta {
            ...
            lore.add("Stored Slayer XP: <light_purple>${Formatting.withCommas(slayer.rng[slayerType]!!.expAccumulated.toBigDecimal(), true)}")
...
fun rngMeter(player: MacrocosmPlayer, slayer: SlayerLevel, slayerType: SlayerType): GUI<ForInventorySixByNine> = kSpigotGUI(GUIType.SIX_BY_NINE) {
    defaultPage = 0
    ...
    page(0) {
        ...
        val compound = createCompound<Pair<Int, SlayerDrop>>({ (dropIndex, drop) ->
            if(drop.requiredLevel > slayer.level) {
                return@createCompound ItemValue.placeholderDescripted(Material.COAL_BLOCK, "<red>???", "Requires ${slayerType.slayer.name} ${drop.requiredLevel}<gray>!")
            }
            ...
            item.meta {
                ...
                if(slayer.rng[slayerType]!!.selectedRngDrop == dropIndex) {
                    ...
                    lore.add(text("<gray>Slayer XP: <light_purple>${Formatting.withCommas(slayer.rng[slayerType]!!.expAccumulated.toBigDecimal(), true)}<dark_purple>/<light_purple>${Formatting.withCommas(expToDrop.toBigDecimal(), true)}").noitalic())
                    lore.add(Component.empty())
        ...
        compound.addContent(slayerType.slayer.drops.indices.associateWith { slayerType.slayer.drops[it] }.toList().filter { it.second.drop.chance < 1 })
    }
}
