...
object Ensure {
    ...
    data class AnnotatedException(val base: Exception?, val additional: String = ""): MacrocosmThrowable(base?.javaClass?.name?.replaceFirstChar(Char::lowercase)?.camelToSnakeCase()?.uppercase() ?: "ASSERTION_FAIL", "While doing a check: ${base?.message ?: "assertion failed"} (${base?.cause ?: "no cause"}); $additional")
}
