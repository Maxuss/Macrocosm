...
object FirstStrikeEnchantment: EnchantmentBase("First Strike", "", 1..5, ItemType.melee(), conflicts = listOf("TRIPLE_STRIKE")) {
    override fun description(level: Int): List<Component> {
        val str = "Increases ${Statistic.DAMAGE.display}<gray> you deal by <green>${Formatting.stats((level * 25f).toBigDecimal(), true)}%<gray> for the first hit on a mob."
...
object TripleStrikeEnchantment: EnchantmentBase("Triple-Strike", "", 1..5, ItemType.melee(), conflicts = listOf("FIRST_STRIKE")) {
    override fun description(level: Int): List<Component> {
        val str = "Increases ${Statistic.DAMAGE.display}<gray> you deal by <green>${Formatting.stats((level * 10f).toBigDecimal(), true)}%<gray> for the first three hits on a mob."
        val reduced = str.reduceToList(25).map { comp("<gray>$it").noitalic() }.toMutableList()
        reduced.removeIf { it.toLegacyString().isBlankOrEmpty() }
        return reduced
    }
