...
enum class Enchant(private val enchant: Enchantment) {
    ...
    // <stat>-steal enchants
    LIFE_STEAL(StealingEnchantment(
        "Life Steal",
        Statistic.HEALTH,
        0.5f,
    ...
    MANA_STEAL(StealingEnchantment(
        "Mana Steal",
        Statistic.INTELLIGENCE,
        0.25f,
        regain = { player, (amount, _) ->
            player.currentMana = kotlin.math.min(player.currentMana + amount, player.calculateStats()!!.intelligence)
        },
        levels = 1..3
    )),
    SYPHON(StealingEnchantment(
        "Syphon",
        Statistic.HEALTH,
        0.2f,
        ...
        regain = { player, (amount, crit) ->
            if(crit) {
                ...
                player.currentHealth = kotlin.math.min(player.currentHealth + (amount * (stats.critDamage.toInt() / 100)), stats.health)
            }
        },
        levels = 1..5,
